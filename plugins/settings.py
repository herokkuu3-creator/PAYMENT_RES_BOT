...
# Define video extensions that SHOULD be standardized to MP4
VIDEO_EXTENSIONS = {
    'mp4', 'mkv', 'avi', 'mov', 'wmv', 'flv', 'webm',
    'mpeg', 'mpg', '3gp', 'm4v'
}
...
async def rename_file(file, sender, edit):
    try:
        ...
        # 1. SAFELY SPLIT FILENAME AND EXTENSION
        base_name, extension = os.path.splitext(file)
        # extension includes the dot, e.g. ".mp4" or ".zip"
        
        # Remove the dot to check against VIDEO_EXTENSIONS
        raw_ext = extension.lstrip('.').lower()
        
        # 2. DETERMINE FINAL EXTENSION
        # Only force MP4 if it is ALREADY a known video format.
        # Otherwise (zip, rar, 7z, pdf), KEEP the original extension.
        if raw_ext in VIDEO_EXTENSIONS:
            final_ext = 'mp4'
        else:
            final_ext = raw_ext if raw_ext else 'mp4' # fallback to mp4 only if no extension exists
        ...
        # Format: "Filename Tag.ext"
        new_file_name = f'{original_file_name} {custom_rename_tag}.{final_ext}'
